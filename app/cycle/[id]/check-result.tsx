...
export default function PhotoCheckResultScreen() {
  ...
  return <ScreenContainer scroll contentStyle={[styles.container, retry && styles.retryContainer]}>
    ...
    <AppCard style={styles.summary}><Text style={styles.label}>NEXT ACTION</Text>{record.result.actions.map((action) => <Text key={action} style={styles.actionText}>{action}</Text>)}{record.result.retakeGuidance ? <Text style={styles.retake}>{record.result.retakeGuidance}</Text> : null}</AppCard>
...
const styles = StyleSheet.create({
  ...
  eyebrow: { ...tokens.typography.label, color: tokens.colors.oliveLabel }, title: { ...tokens.typography.displayLarge, color: tokens.colors.terracottaText }, context: { ...tokens.typography.bodyStrong, color: tokens.colors.ink64, textTransform: "capitalize" }, photo: { aspectRatio: 16 / 10, height: undefined }, summary: { gap: tokens.spacing.sm }, label: { ...tokens.typography.label, color: tokens.colors.oliveLabel }, explanation: { ...tokens.typography.body, color: tokens.colors.ink }, causes: { flexDirection: "row", flexWrap: "wrap", gap: tokens.spacing.xs }, cause: { backgroundColor: tokens.colors.stone, borderRadius: 999, paddingHorizontal: tokens.spacing.sm, paddingVertical: tokens.spacing.xs }, causeText: { ...tokens.typography.caption, color: tokens.colors.alert }, actionText: { ...tokens.typography.cardTitle, color: tokens.colors.forest }, retake: { ...tokens.typography.body, color: tokens.colors.ink82 }, quota: { ...tokens.typography.caption, color: tokens.colors.ink64, textAlign: "center" }
});
