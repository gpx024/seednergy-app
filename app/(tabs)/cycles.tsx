...
import { CycleRow, FeedbackState, ScreenContainer } from "@/src/ui/components";
...
export default function CyclesScreen() {
  ...
  return <ScreenContainer includeBottomSafeArea={false} scroll contentStyle={styles.container}><View style={styles.intro}><Text accessibilityRole="header" style={styles.title}>{t("main.myCycles")}</Text></View>
...
const styles = StyleSheet.create({ container: { gap: tokens.spacing.sectionGap, paddingBottom: tokens.spacing.xs }, intro: { gap: 0 }, title: { ...tokens.typography.displayLarge, color: tokens.colors.brand, marginHorizontal: tokens.spacing.md }, filters: { alignItems: "center", flexDirection: "row", gap: tokens.spacing.md }, filterSurface: { alignItems: "center", flex: 1, justifyContent: "center", minHeight: tokens.layout.size.touchTarget }, filterActiveSurface: { borderRadius: tokens.radii.pill, backgroundColor: tokens.colors.olive, ...tokens.elevation.pillOlive }, filter: { fontFamily: "Inter_600SemiBold", fontSize: 13, lineHeight: 17, color: tokens.colors.olive, textAlign: "center", textTransform: "capitalize" }, filterActive: { color: tokens.colors.raised }, section: { gap: 18 }, label: { ...tokens.typography.label, color: tokens.colors.olive, marginHorizontal: tokens.spacing.md, textTransform: "uppercase" } });
