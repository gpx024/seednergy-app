...
export default function SettingsScreen() {
  ...
  return <ScreenContainer scroll contentStyle={styles.container}>
    <View style={styles.header}><Pressable accessibilityLabel="Back" hitSlop={12} onPress={() => router.back()}><Ionicons color={tokens.colors.ink} name="arrow-back" size={28} /></Pressable><Text accessibilityRole="header" style={styles.title}>Notifications</Text></View>
    ...
    {!loading ? <><AppCard style={styles.toggle}><View style={styles.toggleCopy}><Text style={styles.cardTitle}>Enable notifications</Text><Text style={styles.body}>Get a gentle reminder only when your cycle has a real next action.</Text></View><Switch accessibilityLabel="Enable notifications" onValueChange={(enabled) => setPreferences((current) => ({ ...current, enabled }))} thumbColor={tokens.colors.card} trackColor={{ false: tokens.colors.border, true: tokens.colors.forest }} value={preferences.enabled} /></AppCard>
      ...
      <AppCard variant="nested" style={styles.note}><Text style={styles.label}>Quiet hours</Text><Text style={styles.cardTitle}>{preferences.quietStart} to {preferences.quietEnd}</Text><Text style={styles.body}>Anything due overnight waits until morning. Seednergy sends no generic engagement notifications.</Text></AppCard>
      <AppButton label="Save preferences" loading={saving} onPress={() => void saveAndClose().catch(() => undefined)} /></> : null}
  </ScreenContainer>;
}

const styles = StyleSheet.create({ container: { gap: tokens.spacing.sectionGap }, header: { alignItems: "center", flexDirection: "row", gap: tokens.spacing.md, marginHorizontal: tokens.spacing.md }, title: { ...tokens.typography.displayMedium, color: tokens.colors.terracottaText }, toggle: { alignItems: "center", flexDirection: "row", gap: tokens.spacing.md }, toggleCopy: { flex: 1, gap: tokens.spacing.xs }, cardTitle: { ...tokens.typography.cardTitle, color: tokens.colors.forest }, body: { ...tokens.typography.body, color: tokens.colors.ink82 }, section: { gap: tokens.spacing.sm }, label: { ...tokens.typography.label, color: tokens.colors.oliveLabel, textTransform: "uppercase" }, choice: { alignItems: "center", flexDirection: "row", gap: tokens.spacing.sm }, choiceSelected: { borderColor: tokens.colors.forest, borderWidth: 2 }, choiceCopy: { flex: 1, gap: tokens.spacing.xxs }, choiceTitle: { ...tokens.typography.panelHeadline, color: tokens.colors.ink }, note: { gap: tokens.spacing.sm } });
