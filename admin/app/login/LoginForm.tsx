...
export function LoginForm({ initialError }: { initialError?: string }) {
  ...
  return <div className="card login-card"><Image className="login-brand" src="/brand/wordmark.svg" alt="Seednergy" width={161} height={35} priority /><p className="eyebrow">Private administration</p><h1>Content management</h1><p>Sign in with an approved administrator account.</p><div className="actions"><button className="button" onClick={google}>Continue with Google</button></div><p className="eyebrow login-divider">Or use email</p><form onSubmit={passwordLogin} className="form-grid"><div className="field span-2"><label>Email</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} required /></div><div className="field span-2"><label>Password</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} required /></div><button className="button span-2" type="submit">Sign in</button></form>{message&&<p className="error">{message}</p>}</div>;
}
