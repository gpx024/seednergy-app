...
interface FeedbackStateProps {
  ...
  actionLabel?: string;
  onAction?: () => void;
  titleTone?: "default" | "olive";
}

export function FeedbackState({ kind, title, description, actionLabel, onAction, titleTone = "default" }: FeedbackStateProps) {
  ...
  return (
    <View accessibilityLiveRegion="polite" style={styles.container}>
      ...
      <Text accessibilityRole="header" maxFontSizeMultiplier={1.8} style={[styles.title, titleTone === "olive" && styles.titleOlive]}>{title}</Text>
...
const styles = StyleSheet.create({
  ...
  titleOlive: { color: tokens.colors.forest },
