...
export function BrandHeader({ wordmarkWidth = 156, locationLabel, profileImageUri, onProfilePress }: BrandHeaderProps) {
  return <View accessibilityRole="header" style={styles.header}><View style={styles.copy}><BrandWordmark color={tokens.colors.brand} width={wordmarkWidth} />{locationLabel ? <Text style={styles.location}>{locationLabel}</Text> : null}</View><Pressable accessibilityLabel="Open profile" accessibilityRole="button" onPress={onProfilePress} style={({ pressed }) => [styles.avatarButton, pressed && styles.avatarPressed]}><ProfileAvatar uri={profileImageUri} /></Pressable></View>;
}
...
const styles = StyleSheet.create({
  ...
  location: { fontFamily: "Inter_400Regular", fontSize: 24, lineHeight: 29, color: tokens.colors.olive },
